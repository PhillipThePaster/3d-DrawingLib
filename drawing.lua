...
function Library:New3DPyramid()
    ...
    local function makeNewLines(baseSize, height)
        for l = 1, 4 do
            _lines[l] = nDrawing("Line");
        end;
    end;
    ...
    -- Update Step Function --
    makeNewLines(_pyramid.BaseSize, _pyramid.Height);

    function _pyramid:Update()
        local rot = _pyramid.Rotation or _defaults.Rotation;
        local pos = _pyramid.Position or _defaults.Position;
        local _rotCFrame = nCFrame(pos) * nCFAngles(rad(rot.X), rad(rot.Y), rad(rot.Z));

        local _baseSize = _pyramid.BaseSize or _defaults.BaseSize;
        local _height = _pyramid.Height or _defaults.Height;
        if _baseSize ~= _defaults.BaseSize or _height ~= _defaults.Height then
            makeNewLines(_baseSize, _height);
        end;

        if not _pyramid.Visible then 
            for ln = 1, #_lines do
                _lines[ln].Visible = false;
            end;
        else
            for l = 1, #_lines do
                if _lines[l] then
                    _lines[l].Visible      = _pyramid.Visible      or _defaults.Visible;
                    _lines[l].ZIndex       = _pyramid.ZIndex       or _defaults.ZIndex;
                    _lines[l].Transparency = _pyramid.Transparency or _defaults.Transparency;
                    ...
                    local _previousPosition1, v1 = ToScreen(Camera, p1);
    ...
    local step_Id = "3D_Pyramid"..random_string(10);
    RS:BindToRenderStep(step_Id, 1, _pyramid.Update);

    -- Remove Pyramid --
    function _pyramid:Remove()
        RS:UnbindFromRenderStep(step_Id);
